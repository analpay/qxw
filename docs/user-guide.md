...
`raw` 子命令提供一个**可扩展的调色滤镜插件系统**，用于在 rawpy 解码之后、写盘之前对 RGB 数组做二次调色，模拟不同相机 / 胶片的色彩风格。
...
只要在 `qxw-image raw` 命令执行前（例如在你的项目 `__init__.py` 中）完成 import 触发注册，`--filter my-cool-look` 就会被命令识别。保留名 `default` 不可占用。
...
# 套用富士 Classic Chrome 滤镜（自动禁用嵌入预览、走 rawpy 解码 + 调色）
